...
CREATE TABLE tbturno (
  ...
  -- Auditoria básica do registro (ALCOA+)
  created_at TIMESTAMP WITHOUT TIME ZONE NULL DEFAULT (now() AT TIME ZONE 'America/Fortaleza'),
  created_by INTEGER NULL REFERENCES tbusuario(usuario_id),
  updated_at TIMESTAMP WITHOUT TIME ZONE NULL,
  updated_by INTEGER NULL REFERENCES tbusuario(usuario_id),
  deleted_at TIMESTAMP WITHOUT TIME ZONE NULL,
...
COMMENT ON COLUMN tbturno.codigo IS 'Código do turno (ex: D1, N1, D2, N2)';
COMMENT ON COLUMN tbturno.turno IS 'Nome/descrição do turno';
COMMENT ON COLUMN tbturno.meta_oee IS 'Meta de OEE para o turno (%)';

...
- [x]Preparar ambiente e validar pre-requisitos (AC: 14)
  - [x]Conectar no Supabase do projeto
  - [x]Verificar que migrations anteriores (01-08) foram executadas
  - [x]Verificar que ENUMs necessários existem (tipo_linha_enum, fonte_dados_enum, etc.)

- [x]Criar estrutura base da tabela (AC: 1, 2)
  - [x]Definir tabela `tboee` com PK oee_id INTEGER IDENTITY
  - [x]Adicionar campos de dimensões (linhaproducao_id, produto_id, turno_id, departamento_id, data_referencia, hora_inicio, hora_fim)
  - [x]Adicionar FKs para tblinhaproducao, tbproduto, tbturno, tbdepartamento

- [x]Implementar campos de snapshot de parâmetros (AC: 7)
  - [x]Campo velocidade_nominal NUMERIC(10,2) NOT NULL
  - [x]Campo velocidade_fonte TEXT
  - [x]Campo meta_oee NUMERIC(5,2)

- [x]Implementar campos de tempos (AC: 3)
  - [x]tempo_calendario_minutos INTEGER NOT NULL CHECK >= 0
  - [x]tempo_paradas_estrategicas_minutos INTEGER DEFAULT 0 CHECK >= 0
  - [x]tempo_paradas_planejadas_minutos INTEGER DEFAULT 0 CHECK >= 0
  - [x]tempo_paradas_nao_planejadas_minutos INTEGER DEFAULT 0 CHECK >= 0
  - [x]tempo_pequenas_paradas_minutos INTEGER DEFAULT 0 CHECK >= 0
  - [x]tempo_retrabalho_minutos INTEGER DEFAULT 0 CHECK >= 0

- [x]Implementar campos de tempos calculados via GENERATED (AC: 4, 15)
  - [x]tempo_disponivel_minutos = calendario - estrategicas
  - [x]tempo_operacao_minutos = calendario - estrategicas - planejadas - nao_planejadas
  - [x]tempo_operacional_liquido_minutos = operacao - pequenas_paradas
  - [x]tempo_valioso_minutos = operacional_liquido - retrabalho
  - [x]Validar fórmulas contra docs/project/05-Metodologia-Calculo.md linhas 35-45

- [x]Implementar campos de unidades (AC: 5)
  - [x]unidades_produzidas INTEGER NOT NULL DEFAULT 0 CHECK >= 0
  - [x]unidades_boas INTEGER NOT NULL DEFAULT 0 CHECK >= 0
  - [x]unidades_refugadas INTEGER DEFAULT 0 CHECK >= 0
  - [x]unidades_retrabalhadas INTEGER DEFAULT 0 CHECK >= 0

- [x]Implementar campos de componentes OEE calculados (AC: 6, 15)
  - [x]disponibilidade NUMERIC(5,2) GENERATED = (tempo_operacao / tempo_disponivel) × 100
  - [x]performance NUMERIC(5,2) GENERATED = (tempo_operacional_liquido / tempo_operacao) × 100
  - [x]qualidade_unidades NUMERIC(5,2) GENERATED = (unidades_boas / unidades_produzidas) × 100
  - [x]qualidade_retrabalho NUMERIC(5,2) GENERATED = ((tempo_operacao - tempo_retrabalho) / tempo_operacao) × 100
  - [x]qualidade NUMERIC(5,2) GENERATED = qualidade_unidades × qualidade_retrabalho / 100
  - [x]oee NUMERIC(5,2) GENERATED = disponibilidade × performance × qualidade / 10000
  - [x]atingiu_meta BOOLEAN GENERATED = oee >= meta_oee
  - [x]Validar todas as fórmulas contra docs/project/05-Metodologia-Calculo.md linhas 50-105

- [x]Implementar campos de metadados (AC: 8)
  - [x]calculo_tipo TEXT NOT NULL DEFAULT 'AUTOMATICO'
  - [x]calculo_observacao TEXT
  - [x]calculado_em TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT (now() AT TIME ZONE 'America/Fortaleza')
  - [x]calculado_por INTEGER REFERENCES tbusuario(usuario_id)
  - [x]status TEXT NOT NULL DEFAULT 'ATIVO'
  - [x]invalidado_em, invalidado_por, invalidado_motivo
  - [x]corrigido_por_oee_id INTEGER REFERENCES tboee(oee_id)

- [x]Implementar campos de auditoria ALCOA+ (AC: 9)
  - [x]deletado TEXT DEFAULT 'N'
  - [x]sync TEXT DEFAULT 'N'
  - [x]sync_data TIMESTAMP WITHOUT TIME ZONE
  - [x]created_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT (now() AT TIME ZONE 'America/Fortaleza')
  - [x]created_by INTEGER REFERENCES tbusuario(usuario_id)
  - [x]updated_at TIMESTAMP WITHOUT TIME ZONE
  - [x]updated_by INTEGER REFERENCES tbusuario(usuario_id)
  - [x]deleted_at TIMESTAMP WITHOUT TIME ZONE
  - [x]deleted_by INTEGER REFERENCES tbusuario(usuario_id)

- [x]Implementar constraints (AC: 10)
  - [x]CONSTRAINT tboee_pkey PRIMARY KEY (oee_id)
  - [x]CONSTRAINT ck_unidades_boas CHECK (unidades_boas <= unidades_produzidas)
  - [x]CONSTRAINT ck_unidades_perdas CHECK ((unidades_refugadas + unidades_retrabalhadas) <= unidades_produzidas)
  - [x]CONSTRAINT ck_horario_oee CHECK (hora_fim IS NULL OR hora_fim >= hora_inicio)
  - [x]CONSTRAINT uq_oee_periodo UNIQUE (linhaproducao_id, data_referencia, turno_id, hora_inicio)

- [x]Criar índices para performance (AC: 11)
  - [x]CREATE INDEX idx_tboee_linha_data ON tboee(linhaproducao_id, data_referencia)
  - [x]CREATE INDEX idx_tboee_departamento_data ON tboee(departamento_id, data_referencia)
  - [x]CREATE INDEX idx_tboee_produto ON tboee(produto_id)
  - [x]CREATE INDEX idx_tboee_turno ON tboee(turno_id)
  - [x]CREATE INDEX idx_tboee_lote ON tboee(lote_id)
  - [x]CREATE INDEX idx_tboee_status ON tboee(status) WHERE status = 'ATIVO'
  - [x]CREATE INDEX idx_tboee_calculado_em ON tboee(calculado_em)
  - [x]CREATE INDEX idx_tboee_atingiu_meta ON tboee(atingiu_meta) WHERE atingiu_meta IS NOT NULL

- [x]Implementar regras APPEND-ONLY (AC: 12)
  - [x]CREATE RULE tboee_no_update AS ON UPDATE TO tboee DO INSTEAD NOTHING
  - [x]CREATE RULE tboee_no_delete AS ON DELETE TO tboee DO INSTEAD NOTHING
  - [x]Adicionar comentários explicativos nas rules

- [x]Adicionar comentários de documentação (AC: 13)
  - [x]COMMENT ON TABLE tboee
  - [x]COMMENT ON COLUMN tboee.oee_id
  - [x]COMMENT ON COLUMN tboee.velocidade_nominal (explicar que é snapshot)
  - [x]COMMENT ON COLUMN tboee.meta_oee (explicar que é snapshot)
  - [x]COMMENT ON COLUMN tboee.calculo_tipo
  - [x]COMMENT ON COLUMN tboee.status
  - [x]COMMENT ON COLUMN tboee.corrigido_por_oee_id

- [x]Executar migration e validar (AC: 14, 15)
  - [x]Executar script 09-tboee-snapshot.sql no Supabase
  - [x]Verificar que tabela foi criada sem erros
  - [x]Validar estrutura via `\d tboee`
  - [x]Validar índices via `\di tboee*`
  - [x]Validar rules via `\d+ tboee`
  - [x]Testar INSERT de registro de exemplo (sucesso)
  - [x]Testar UPDATE de registro (deve falhar - rule bloqueando)
  - [x]Testar DELETE de registro (deve falhar - rule bloqueando)
  - [x]Validar que campos GENERATED são calculados corretamente

...
- [x]Preparar ambiente e validar pre-requisitos (AC: 12)
  ...
  - [x]Verificar que tbvelocidadenominal e tbmetaoee existem (podem estar vazias)
...
- [x]Buscar meta_oee_vigente (AC: 9)
  - [x]Subquery: SELECT meta_oee FROM tbmetaoee WHERE linha_id = lp.linhaproducao_id AND data_producao BETWEEN data_inicio_vigencia AND COALESCE(data_fim_vigencia, '9999-12-31') AND deletado = 'N' ORDER BY data_inicio_vigencia DESC LIMIT 1
